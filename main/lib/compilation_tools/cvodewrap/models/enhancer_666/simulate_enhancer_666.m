% simulate_enhancer_666.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_666(tout,theta)
% [...] = simulate_enhancer_666(tout,theta,kappa,options)
% sol = simulate_enhancer_666(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_666(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_666(varargin)
...
options_cvode.nx = 9; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 36; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),9);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),9);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),9);
sol.J = zeros(9,9);
sol.dydx = zeros(2,9);
sol.dydp = zeros(2,2);
...
sol.dxdotdp = zeros(9,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),9,length(options_cvode.sens_ind));
...
rt = [5  6  3  7  9  1  8  4  2];
cw_enhancer_666(sol,tout,theta(1:2),kappa(1:19),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
