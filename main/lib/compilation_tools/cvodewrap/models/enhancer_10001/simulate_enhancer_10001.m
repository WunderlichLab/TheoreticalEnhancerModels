% simulate_enhancer_10001.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_10001(tout,theta)
% [...] = simulate_enhancer_10001(tout,theta,kappa,options)
% sol = simulate_enhancer_10001(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_10001(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_10001(varargin)
...
options_cvode.sens_ind = 1:4;
options_cvode.nx = 20; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 110; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 16;

options_cvode.lbw = 18;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),20);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),20);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),20);
sol.J = zeros(20,20);
sol.dydx = zeros(2,20);
sol.dydp = zeros(2,4);
...
sol.dxdotdp = zeros(20,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),20,length(options_cvode.sens_ind));
...
if(max(options_cvode.sens_ind)>4)
    error('Sensitivity index exceeds parameter dimension!')
end
rt = [2  12  13  14   7   1  15  16   4   3  20   5  10  11  17  18  19   6   9   8];
cw_enhancer_10001(sol,tout,theta(1:4),kappa(1:41),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
