% simulate_enhancer_667.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_667(tout,theta)
% [...] = simulate_enhancer_667(tout,theta,kappa,options)
% sol = simulate_enhancer_667(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_667(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_667(varargin)
...
options_cvode.sens_ind = 1:2;
options_cvode.nx = 14; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 74; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 13;

options_cvode.lbw = 11;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),14);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),14);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),14);
sol.J = zeros(14,14);
sol.dydx = zeros(2,14);
sol.dydp = zeros(2,2);
...
sol.dxdotdp = zeros(14,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),14,length(options_cvode.sens_ind));
...
if(max(options_cvode.sens_ind)>2)
    error('Sensitivity index exceeds parameter dimension!')
end
rt = [7   8  14   9   1  10  11   3   4   2  12   5  13   6];
cw_enhancer_667(sol,tout,theta(1:2),kappa(1:29),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
