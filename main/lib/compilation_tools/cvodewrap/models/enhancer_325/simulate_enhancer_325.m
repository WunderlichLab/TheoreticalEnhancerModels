% simulate_enhancer_325.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_325(tout,theta)
% [...] = simulate_enhancer_325(tout,theta,kappa,options)
% sol = simulate_enhancer_325(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_325(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_325(varargin)
...
options_cvode.nx = 135; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 2400; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 134;

options_cvode.lbw = 134;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),135);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),135);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),135);
sol.J = zeros(135,135);
sol.dydx = zeros(2,135);
sol.dydp = zeros(2,11);
...
sol.dxdotdp = zeros(135,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),135,length(options_cvode.sens_ind));
...
rt = [75   76   77   78   79   80   81   82   83   84   85   86   87   88   71   51   53   56   54   49   65    2   89   90   44   45   48   91   92   74   52   55   69   66   57    6    3    5   93   94   46   95   96   97   50   67   68   62   13    8    7   98   99   47  100  101  102   58   59   64    4   14  103    1   15  135  104  105  106   60   63    9   12  107   11   16  108  109  110  111   61  112   10   17  113   19   18  114  115  116   22   24   27   25   20   36  117  118  119   23   26   40   37   28  120  121  122   21   38   39   33  123  124  125   29   30   35  126  127  128   31   34  129  130  131   32  132  133  134   41   42   73   43   72   70];
cw_enhancer_325(sol,tout,theta(1:11),kappa(1:271),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
