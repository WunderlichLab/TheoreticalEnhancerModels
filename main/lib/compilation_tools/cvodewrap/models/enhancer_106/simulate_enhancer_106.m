% simulate_enhancer_106.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_106(tout,theta)
% [...] = simulate_enhancer_106(tout,theta,kappa,options)
% sol = simulate_enhancer_106(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_106(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_106(varargin)
...
options_cvode.nx = 119; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 1587; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 118;

options_cvode.lbw = 117;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),119);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),119);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),119);
sol.J = zeros(119,119);
sol.dydx = zeros(2,119);
sol.dydp = zeros(2,6);
...
sol.dxdotdp = zeros(119,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),119,length(options_cvode.sens_ind));
...
rt = [82   83   84   85   86   87   88   89   90   91   92   93   94    2   20   25   26   21    7   13   14    4   33   39   40   30   95   96   22   24   27   11    8   10   15   37   34   36   41   97   98   23   28   12    5    6   18   38   31   32   44   99  100   19    9   16   17    3   35   42   43   29  101  102   47   52   53   48   70   76   77   67  103  104   49   51   54   74   71   73   78  105  106   50   55   75   68   69   81  107  108   46   72   79   80   56  109  119   58   63   64   59  110  111   60   62   65  112  113   61   66  114  115   57  116  117   45  118    1];
cw_enhancer_106(sol,tout,theta(1:6),kappa(1:239),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
