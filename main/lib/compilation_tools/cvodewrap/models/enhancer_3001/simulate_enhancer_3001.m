% simulate_enhancer_3001.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_3001(tout,theta)
% [...] = simulate_enhancer_3001(tout,theta,kappa,options)
% sol = simulate_enhancer_3001(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_3001(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_3001(varargin)
...
options_cvode.nx = 27; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 247; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 25;

options_cvode.lbw = 25;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),27);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),27);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),27);
sol.J = zeros(27,27);
sol.dydx = zeros(2,27);
sol.dydp = zeros(2,6);
...
sol.dxdotdp = zeros(27,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),27,length(options_cvode.sens_ind));
...
rt = [15  16  17  18  19   7   1  20  21  22  23   8   2   3  24  27  11   4  25  26   9  13  14  12   5  10   6];
cw_enhancer_3001(sol,tout,theta(1:6),kappa(1:55),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
