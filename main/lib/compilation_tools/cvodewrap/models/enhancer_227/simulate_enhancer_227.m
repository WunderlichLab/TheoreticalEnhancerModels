% simulate_enhancer_227.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_227(tout,theta)
% [...] = simulate_enhancer_227(tout,theta,kappa,options)
% sol = simulate_enhancer_227(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_227(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_227(varargin)
...
options_cvode.nx = 44; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 369; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 43;

options_cvode.lbw = 38;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),44);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),44);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),44);
sol.J = zeros(44,44);
sol.dydx = zeros(2,44);
sol.dydp = zeros(2,11);
...
sol.dxdotdp = zeros(44,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),44,length(options_cvode.sens_ind));
...
rt = [28  29  30  44  31  32  33  34  16  21  25  35  36  26  18  22  19  37  38  39  40  20  41   4   5   7   6  24  13  10  11   8  42  14   2   9  43  15   3  27  12  17  23   1];
cw_enhancer_227(sol,tout,theta(1:11),kappa(1:89),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
