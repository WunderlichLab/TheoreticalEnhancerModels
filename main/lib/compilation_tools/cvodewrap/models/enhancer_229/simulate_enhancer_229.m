% simulate_enhancer_229.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_229(tout,theta)
% [...] = simulate_enhancer_229(tout,theta,kappa,options)
% sol = simulate_enhancer_229(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_229(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_229(varargin)
...
options_cvode.nx = 54; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 452; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 51;

options_cvode.lbw = 52;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),54);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),54);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),54);
sol.J = zeros(54,54);
sol.dydx = zeros(2,54);
sol.dydp = zeros(2,11);
...
sol.dxdotdp = zeros(54,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),54,length(options_cvode.sens_ind));
...
rt = [36  37  38  39  40  41  42  43  44   2   3  10  15  31  45  16  30   5   4  11  13  46   1  17  47  48   6   7  33  54  12  32  19   8  49  20  14  50  51  21  26  35  23  27  24  52  25  53   9  34  18  22  28  29];
cw_enhancer_229(sol,tout,theta(1:11),kappa(1:109),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
