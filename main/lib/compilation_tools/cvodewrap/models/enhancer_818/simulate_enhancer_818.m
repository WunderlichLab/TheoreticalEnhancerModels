% simulate_enhancer_818.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_enhancer_818(tout,theta)
% [...] = simulate_enhancer_818(tout,theta,kappa,options)
% sol = simulate_enhancer_818(...)
% [status,tout,x,y,sx,sy] = simulate_enhancer_818(...)
%
% INPUTS:
% =======
...
function varargout = simulate_enhancer_818(varargin)
...
options_cvode.nx = 104; % MUST NOT CHANGE THIS VALUE
...
options_cvode.nnz = 1420; % MUST NOT CHANGE THIS VALUE
...
options_cvode.ubw = 102;

options_cvode.lbw = 102;

options_cvode.ss = 0;
options_cvode.qPositiveX = zeros(length(tout),104);

sol.status = 0;
sol.t = tout;
sol.x = zeros(length(tout),104);
sol.y = zeros(length(tout),2);
sol.xdot = zeros(length(tout),104);
sol.J = zeros(104,104);
sol.dydx = zeros(2,104);
sol.dydp = zeros(2,11);
...
sol.dxdotdp = zeros(104,options_cvode.np);
plist = options_cvode.sens_ind-1;
if(options_cvode.sensi>0)
    sol.xS = zeros(length(tout),104,length(options_cvode.sens_ind));
...
rt = [44   57   73   74   75   76   77   78   79   71   45   80   63   33   38   29   24   25   46   26   47   48   23   35   30   49   36   28   56   58   40   59   31   32   55   37   62   60    1   81   82    2   83    3    4  104   27   84   85    5    6   86    7   87   88   16   50   89   90    8   91    9   92   93   12   51   94   95   13   96   18   14   66   41   97   67   10   98   99   17   52  100  101   19   20   65   43  102   64   15   68   42  103   69   21   54   72   70   34   53   39   11   61   22];
cw_enhancer_818(sol,tout,theta(1:11),kappa(1:209),options_cvode,plist,pbar,xscale);
sol.x = sol.x(:,rt);
